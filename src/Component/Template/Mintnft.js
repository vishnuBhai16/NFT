...
const Mintnft = () => {
  ...
  const mintNft = async () => {
    ...
    try {
      ...
      if (Number(100) > C_allowance) {
        await token.methods
          .approve(
            nft._address,
            web3.utils.toWei(Number(100) - C_allowance, "ether")
          )
          .send({ from: account });
      }
      await nft.methods.mintNft(JSON.stringify({...Nft,image:Nft.URL})).send({ from: account });
      alert("Nft Minted");
      setNft({ name: "", description: "", URL: "" });
      setloading(false);
    } catch (error) {
      setloading(false);
              ...
              <div className="nft-card card shadow-sm">
                <div className="card-body">
                  <div className="img-wrap">
                    <img src={Nft.URL ? Nft.URL : "img/bg-img/17.jpg"} alt="" />
                    {/* Badge */}
                  </div>
                  {/* Others Info */}
                  {/* Meta Info */}
